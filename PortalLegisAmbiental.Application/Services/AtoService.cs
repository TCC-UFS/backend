...
namespace PortalLegisAmbiental.Application.Services
{
    public class AtoService : IAtoService
    {
        ...
        public async Task<List<AtoResponse>> Search(string? numero = null, string? tipo = null, string? jurisdicao = null, string order = "desc", int page = 1, int limit = 10)
        {
            if (page <= 0 || limit <= 0)
            ...
            if (isEnum)
                atos = await _atoRepository.Search(numero: numero, tipo: tipo, ambito: ambito, includeTipo: true, includeJurisdicao: true, order: order, page: page, limit: limit);
            else
                atos = await _atoRepository.Search(numero: numero, tipo: tipo, jurisdicao: jurisdicao, includeTipo: true, includeJurisdicao: true, order: order, page: page, limit: limit);
